...
function LiveMapPage() {
  ...
  const emergencyCount = alerts.filter((a) => a.status === 'active' || a.status === 'escalated').length;
  const activeUnitCount = units.filter((u) => u.status !== 'offline').length;

  // Map center: use first active alert's location or default
  const mapCenter = alerts.length > 0 && alerts[0].latitude && alerts[0].longitude
  ...
  const renderMap = () => {
      ...
      return (
        <div style={{
          ...
          <div style={{ fontSize: '14px', color: '#666', maxWidth: '350px', lineHeight: '1.6' }}>
            Add your Google Maps API key to <code>.env</code> file:<br />
    ...
    return (
      ...
      >
        {/* Emergency markers */}
        {activeView === 'Emergencies' && alerts.map((alert) => {
        ...
        {/* Unit markers */}
        {activeView === 'Units' && units.map((unit) => {
          if (!unit.location?.latitude || !unit.location?.longitude) return null;
          return (
            <Marker
              key={unit.id}
              position={{ lat: Number(unit.location.latitude), lng: Number(unit.location.longitude) }}
              icon={{
                url: 'data:image/svg+xml;charset=UTF-8,' + encodeURIComponent(
          ...
          <InfoWindow
            position={
              selectedMarker.type === 'alert'
                ? { lat: Number(selectedMarker.data.latitude), lng: Number(selectedMarker.data.longitude) }
                : { lat: Number(selectedMarker.data.location?.latitude), lng: Number(selectedMarker.data.location?.longitude) }
            }
      ...
      <div style={{ display: 'flex', gap: '20px', height: 'calc(100vh - 200px)' }}>
        <div style={{ flex: 2, position: 'relative' }}>
          ...
          <div style={{
            ...
            <div className="card" style={{ minWidth: '130px' }}>
              <div style={{ fontSize: '32px', fontWeight: 'bold', marginBottom: '5px' }}>{emergencyCount}</div>
              <div style={{ fontSize: '12px', color: '#666' }}>Emergencies</div>
            </div>
            <div className="card" style={{ minWidth: '130px' }}>
              <div style={{ fontSize: '32px', fontWeight: 'bold', marginBottom: '5px' }}>{activeUnitCount}</div>
        ...
        <div style={{ flex: 1, background: 'white', borderRadius: '12px', padding: '20px', overflowY: 'auto' }}>
          ...
          {activeView === 'Emergencies' ? (
            alerts.length === 0 ? (
              <div style={{ textAlign: 'center', padding: '20px', color: '#999' }}>No active emergencies</div>
            ) : (
              alerts.map((alert) => {
                const uid = alert.userId || alert.ownerId;
                const userData = userCache[uid];
            ...
            units.filter((u) => u.status !== 'offline').length === 0 ? (
              <div style={{ textAlign: 'center', padding: '20px', color: '#999' }}>No active units</div>
            ) : (
                ...
                return (
                  ...
                  >
                    <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '10px' }}>
                      ...
                      <span className="badge" style={{ background: statusColors[unit.status] || '#666', fontSize: '10px' }}>
