...
// If you're using Expo (app.json present), @expo/vector-icons is available by default
// and avoids extra installs. Falls back gracefully if removed.
import { MaterialCommunityIcons } from '@expo/vector-icons';

const Home = ({ navigation }) => {
  const handleGetStarted = () => {
    // Navigate if a route exists, otherwise no-op
    ...
    <SafeAreaView style={styles.safe}>
      <ScrollView contentContainerStyle={styles.container} showsVerticalScrollIndicator={false}>
        ...
        <TouchableOpacity activeOpacity={0.9} onPress={handleGetStarted} style={styles.cta}>
          <Text style={styles.ctaText}>Start Protecting Today <Text style={styles.ctaArrow}>→</Text></Text>
        </TouchableOpacity>
      </ScrollView>
    </SafeAreaView>
  );
...
const styles = StyleSheet.create({
  ...
  iconCircle: {
    ...
    alignItems: 'center',
    justifyContent: 'center',
    backgroundColor: '#4F2CF5', // rich purple/blue
    shadowColor: '#000',
    shadowOpacity: 0.15,
    shadowRadius: 10,
