...
import { SafeAreaView, ScrollView, View, Text, StyleSheet, TouchableOpacity, KeyboardAvoidingView, Platform, Alert, TextInput, ActivityIndicator } from 'react-native';
	...
	const handleLogin = async () => {
		...
		try {
			...
			if (!snap.exists()) {
				...
				if (navigation?.navigate) navigation.navigate(fallbackRole === 'guardian' ? 'GuardianDashboard' : 'Dashboard');
				return;
			}
			const role = snap.data().role;
			console.log('[Login] Loaded role:', role);
			if (navigation?.navigate) navigation.navigate(role === 'guardian' ? 'GuardianDashboard' : 'Dashboard');
		} catch (e) {
			console.error('[Login] Error during login:', e);
			let message = e?.message ?? 'Please try again.';
			if (e?.code === 'auth/invalid-credential' || e?.code === 'auth/wrong-password') message = 'Incorrect password.';
				...
				<ScrollView contentContainerStyle={styles.scroll} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
					<View style={styles.card}>
						...
						<View style={styles.tabs}>
							...
							>
								<Text style={[styles.tabText, activeTab === TAB_TYPES.USER && styles.tabTextActive]}>User Login</Text>
							</TouchableOpacity>
							<TouchableOpacity
								activeOpacity={0.8}
							...
							>
								<Text style={[styles.tabText, activeTab === TAB_TYPES.GUARDIAN && styles.tabTextActive]}>Guardian Login</Text>
							</TouchableOpacity>
						</View>
						...
						<View style={styles.section}>
							...
							<View style={styles.passwordContainer}>
								...
								<TouchableOpacity onPress={() => setShowPassword(!showPassword)} style={styles.eyeIcon} hitSlop={{ top: 10, bottom: 10, left: 10, right: 10 }}>
									<MaterialCommunityIcons name={showPassword ? 'eye-off' : 'eye'} size={22} color="#555" />
								</TouchableOpacity>
							</View>
						...
						<View style={styles.registerWrap}>
							<Text style={styles.registerPrompt}>New User? <Text onPress={goRegister} style={styles.registerLink}>Register Now</Text></Text>
						</View>
					</View>
				</ScrollView>
